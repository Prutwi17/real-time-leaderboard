...
| leaderboard-service | **Implemented:** Redis Sorted Sets for live ranking (all-time per sport); Kafka consumer for `leaderboard.score.submitted` topic (idempotent on eventId); WebSocket/STOMP broadcasting via `/ws` endpoint (SockJS fallback); rebuild-from-history; paginated/top-N/rank/nearby read endpoints. | Daily/weekly/season windows (planned) |
